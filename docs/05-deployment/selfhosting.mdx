...
[StudyU CLI](https://github.com/hpi-studyu/studyu/tree/dev/docker#quick-start-with-the-studyu-cli) to start and
...
Supabase itself is made up of different services (you can learn more about them
 [here](https://supabase.com/docs/guides/self-hosting#architecture), and all of these services are
...
## Get Started with our StudyU CLI

We have developed a StudyU CLI tool to streamline the process of setting up StudyU. Since there are multiple ways how to
setup and configure a self-hosted version of StudyU, more guidance is available in the `docker/README.txt` of the
...
2. ~~If you want to run a productive environment, checkout the `master` branch.~~

~~`git checkout master`~~

3. Switch to the `docker` directory inside the studyu directory.

`cd studyu/docker`

4. Configure the StudyU CLI.

`./studyu-cli config`

5. Start your individual setup.
